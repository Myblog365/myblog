...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid = '', $appsecret = '', $token = '') {
		...
		if ($result) {
			...
			S ( $authname, $this->access_token, 3600 );
			return $this->access_token;
		}
		return false;
	}
	 ...
	 *
	 * @param array $data
	 *        	消息结构{ "touser":[ "OPENID1", "OPENID2" ], "mpnews":{ "media_id":"123dsdajkasd231jhksad" }, "msgtype":"mpnews" }
	 * @return boolean|array
	 */
	 ...
	 *
	 * @param array $data
	 *        	消息结构{ "filter":[ "group_id": "2" ], "mpnews":{ "media_id":"123dsdajkasd231jhksad" }, "msgtype":"mpnews" }
	 * @return boolean|array
	 */
	 ...
	 * @return string
	 */
	public function createNativePackage($ret_code = 0, $ret_err_msg = 'ok', $out_trade_no, $body, $total_fee, $notify_url, $spbill_create_ip, $fee_type = 1, $bank_type = "WX", $input_charset = "UTF-8", $time_start = "", $time_expire = "", $transport_fee = "", $product_fee = "", $goods_tag = "", $attach = "") {
		$nativeObj["AppId"] = $this->appid;
		$nativeObj["appkey"] = $this->paysignkey;
	 ...
	 * 回调通知签名验证
	 *
	 * @param array $orderxml
	 *        	返回的orderXml的数组表示，留空则自动从post数据获取
	 * @return boolean
	 */
	 ...
	 * 获取多客服在线客服接待信息
	 *
	 * @return boolean|array 1：pc在线，2：手机在线,若pc和手机同时在线则为 1+2=3
	 *         "kf_id": "1001",		//客服工号
