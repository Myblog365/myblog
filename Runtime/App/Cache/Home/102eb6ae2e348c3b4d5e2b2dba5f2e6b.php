...
<ul id="author-message"><li class="tip">与 <?php echo (get_nickname($to_uid)); ?> 对话</li>
<li>

<form method="post" target="formAjax" noenter action="<?php echo U('Ucenter/sendmess');?>">
<input type="hidden" name="to_uid" value="<?php echo ($to_uid); ?>">
<input type="hidden" name="from_uid" value="<?php echo ($from_uid); ?>">
<input type="hidden" name="type" value="1">
<p><textarea class="form-control" rows="3" name="content"></textarea></p>
<p class="clearfix">
<a class="btn btn-link pull-left" href="<?php echo U('Ucenter/usermail');?>">查看我的消息</a>
<button type="submit" class="btn btn-primary pull-right">确定发送</button></p></form>


</li>

<?php if(is_array($tomaillist)): $i = 0; $__LIST__ = $tomaillist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li id="<?php echo ($vo["id"]); ?>"><div class="message-content clearfix">

<?php if($vo['from_uid'] == is_login()): ?><a class="pull-right" href="<?php echo ($vo['fromuser']['space_url']); ?>">
<img src="<?php echo ($vo['fromuser']['avatar64']); ?>" alt="<?php echo ($vo['fromuser']['nickname']); ?>" title="<?php echo ($vo['fromuser']['nickname']); ?>" class="avatar" width="34" height="34"></a>
<?php else: ?>
<a class="pull-left" href="<?php echo ($vo['touser']['space_url']); ?>">
<img src="<?php echo ($vo['fromuser']['avatar64']); ?>" title="<?php echo ($vo['fromuser']['nickname']); ?>" alt="<?php echo ($vo['fromuser']['nickname']); ?>" class="avatar" width="34" height="34"></a><?php endif; ?>

<div class="pm-box"><div class="pm-content <?php if(($vo['is_read'] == 0) AND ($vo['from_uid'] != is_login())): ?>highlight<?php endif; ?>"><?php echo ($vo['content']); ?></div>
<p class="pm-date"><?php echo (toDate($vo["create_time"])); ?></p></div></div></li><?php endforeach; endif; else: echo "" ;endif; ?>
