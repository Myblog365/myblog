...
<div id="main" class="settings col-md-8 form-horizontal">
          ...
          <div class="clearfix stream-list border-top board">
           <?php $map=array($cxuser['uid'],'10','',);$data = callApi("Focus/getTag",$map);$__LIST__ = $data['data']; if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="media stream-list__item">
	<a href="<?php echo ZSU('/tagart/'.$vo['tag']['id'],'Index/tagart',array('id'=>$vo['tag']['id']));?>" class="pull-left">
		<img src="<?php if(!empty($vo['tag']['img'])): ?>/thinkblog<?php endif; echo ($vo["tag"]["path"]); ?>" alt="<?php echo ($vo["tag"]["title"]); ?>" class="media-object avatar-40">
	</a>
	<div class="media-body pull-left">
		<h4 class="h5 media-heading">
			<a href="<?php echo ZSU('/tagart/'.$vo['tag']['id'],'Index/tagart',array('id'=>$vo['tag']['id']));?>"><?php echo ($vo["tag"]["title"]); ?></a>
		</h4>
		<div class="media-content text-muted">
			<?php echo ($vo["tag"]["des"]); ?>		</div>
		
	</div>
	  
	            <?php if(!$vo['hasfocustag']): ?><a callback="focusevent" id="focusevent<?php echo ($vo['tag']['id']); ?>" href="<?php echo U('Index/guanzhu',array('id'=>$vo['tag']['id'],'type'=>2));?>" target=AjaxTodo class="btn btn-success btn-xs pull-right" alt="取消关注" >加关注</a>
                           <?php else: ?>
                            <a callback="focusevent" id="focusevent<?php echo ($vo['tag']['id']); ?>" href="<?php echo U('Index/guanzhu',array('id'=>$vo['tag']['id'],'type'=>2));?>" target=AjaxTodo class="btn btn-default btn-xs pull-right" alt="加关注" >取消关注</a><?php endif; ?> 
...
<div class="text-center">
               <ul class="pagination">
<?php $map=array($cxuser['uid'],);$data = callApi("Focus/getTagCount",$map);$count = $data['data'];$__PAGE__ = new \Think\Page($count, 10);echo $__PAGE__->show(); ?>
            </ul>   
</div>
