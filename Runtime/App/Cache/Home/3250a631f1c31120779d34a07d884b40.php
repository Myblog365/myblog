...
<div id="main" class="settings col-md-8 form-horizontal">
          ...
          <div class="clearfix stream-list border-top board">
           <?php $map=array($cxuser['uid'],'10','',);$data = callApi("Focus/getUser",$map);$__LIST__ = $data['data']; if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="media stream-list__item">
	<a href="<?php echo ($vo["user"]["space_url"]); ?>" class="pull-left">
		<img src="<?php echo ($vo["user"]["avatar64"]); ?>" alt="<?php echo ($vo["user"]["nickname"]); ?>" class="media-object avatar-40">
	</a>
	<div class="media-body pull-left">
		<h4 class="h5 media-heading">
			<a href="<?php echo ($vo["user"]["space_url"]); ?>"><?php echo ($vo["user"]["nickname"]); ?></a><small class="ml20"><?php if($vo['hufen']): ?>[<i class="icon-exchange"></i> 已相互关注]<?php endif; ?></small>
		</h4>
		<div class="media-content text-muted">
			<?php echo ($vo["user"]["score"]); ?> 积分 · <?php echo ($vo["user"]["zan"]); ?> 次被赞 · <?php echo ($vo["user"]["artnum"]); ?> 篇文章		</div>
		
	</div>
	  
	            <?php if(!$vo['hasfocususer']): ?><a callback="focusevent" id="focusevent<?php echo ($vo["user"]["uid"]); ?>" href="<?php echo U('Index/guanzhu',array('id'=>$vo['user']['uid'],'type'=>0));?>" target=AjaxTodo class="btn btn-success btn-xs pull-right" alt="取消关注" >加关注</a>
                           <?php else: ?>
                            <a callback="focusevent" id="focusevent<?php echo ($vo["user"]["uid"]); ?>" href="<?php echo U('Index/guanzhu',array('id'=>$vo['user']['uid'],'type'=>0));?>" target=AjaxTodo class="btn btn-default btn-xs pull-right" alt="加关注" >取消关注</a><?php endif; ?> 
...
<div class="text-center">
               <ul class="pagination">
<?php $map=array($cxuser['uid'],);$data = callApi("Focus/getUserCount",$map);$count = $data['data'];$__PAGE__ = new \Think\Page($count, 10);echo $__PAGE__->show(); ?>
            </ul>   
</div>
