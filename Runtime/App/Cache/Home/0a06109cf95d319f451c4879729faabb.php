...
<?php if($user_auth['uid'] > 0): ?><ul class="opts pull-right list-inline">
	...
	<div class="mt10 ml20"><a href="<?php echo U('Ucenter/artadd');?>"
		class="btn btn-primary">发布 </a></div>
            ...
            <div class="row">
                <div class="col-md-9">
                    <ol class="breadcrumb">
                        ...
                        <li><a href="<?php echo ($authorinfo['space_url']); ?>"><?php echo ($authorinfo['nickname']); ?></a></li>
                        <li class="active">文章详情</li>
                    </ol>
                    <h1 class="h3 title art_tt" id="articleTitle"><a href="<?php echo ZSU('/artc/'.$info['id'],'Index/artc',array('id'=>$info['id']));?>"> <?php echo ($info["title"]); ?></a></h1>
                    <div class="author" data-username="GitCafe" data-userslug="gitcafe" data-useravatar="http://sfault-avatar.b0.upaiyun.com/380/542/3805425491-5449782ad67ea_huge128">
                        <a href="<?php echo ($authorinfo['space_url']); ?>" class="mr5"><img class="avatar-24 mr10" src="<?php echo ($authorinfo['avatar64']); ?>" alt="<?php echo ($authorinfo['nickname']); ?>"><strong><?php echo ($authorinfo['nickname']); ?></strong></a> <strong class="text-black mr10"></strong>
                        <?php echo (friendlyDate($info["create_time"])); ?> 发布于[<a href="<?php echo ZSU('/artlist/'.$info['cid'],'Index/artlist',array('cid'=>$info['cid']));?>"><?php echo (get_cate_nameByid($info["cid"])); ?></a>]
                                                                    </div>
                </div>
                <div class="col-md-3">
                    <ul class="widget-action--ver list-unstyled">
                        <li>
                                                                                            <a callback="dingcai" href="<?php echo U('Index/ding',array('id'=>$info['id']));?>" target=AjaxTodo class="btn btn-success btn-sm" alt="赞">赞</a>
                                                                                        <strong id="ding"><?php echo ($info["ding"]); ?></strong> 赞
                        </li>
                        <li>
                                                                                                <?php if($hassc): ?><a callback="focusnum" id="focusnum<?php echo ($info['id']); ?>" href="<?php echo U('Index/focusnum',array('id'=>$info['id'],'mark'=>'sideBookmarked','type'=>1));?>" target=AjaxTodo class="btn btn-default btn-sm" alt="收藏">取消收藏</a>
                                                                                                <?php else: ?>
                                                                                                <a callback="focusnum" id="focusnum<?php echo ($info['id']); ?>"  href="<?php echo U('Index/focusnum',array('id'=>$info['id'],'mark'=>'sideBookmarked','type'=>1));?>" target=AjaxTodo class="btn btn-success btn-sm" alt="取消收藏">收藏</a><?php endif; ?>
                                                                                        
                                                                                        <strong id="sideBookmarked"><?php echo ($sccount); ?></strong> 收藏，<strong class="no-stress"><?php echo ($info["view"]); ?></strong> 浏览
                        </li>
                    </ul>
                </div>
            </div>
...
<p class="mt30"><?php echo ($info["copyright"]); ?>
</p>
<?php echo hook('documentDetailAfter');?>
                ...
                <div class="clearfix">
                    ...
                    <?php if(is_array($info['tagarr'])): $i = 0; $__LIST__ = $info['tagarr'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tagarr): $mod = ($i % 2 );++$i;?><li class="tagPopup">
                                           <?php echo ($tagarr); ?>
                ...
                <?php $map=array($info,'cate',);$data = callApi("Art/getPreArt",$map);$vo = $data['data']; if(!empty($vo)): ?><li class="pull-left">上一篇：<a href="<?php echo ZSU('/artc/'.$vo['id'],'Index/artc',array('id'=>$vo['id']));?>" rel="prev"><?php echo (cutstr_html($vo["title"],15)); ?></a></li><?php endif; ?>
                   <?php $map=array($info,'cate',);$data = callApi("Art/getNextArt",$map);$vo = $data['data']; if(!empty($vo)): ?><li class="pull-right">下一篇：<a href="<?php echo ZSU('/artc/'.$vo['id'],'Index/artc',array('id'=>$vo['id']));?>"><?php echo (cutstr_html($vo["title"],15)); ?></a></li><?php endif; ?>                   
               </ul>
             

                <h2 class="h4 post-title">讨论区</h2>
  <div class="panel panel-default" id="comments">
                                <?php echo hook('Advs', 'up_commentbox');?>
<?php echo hook('localComment', array('app'=>'Home','con'=>'Article','id'=>$info['id'], 'uid'=>$userinfo['uid']));?>
                 ...
                 <div class="widget-box clearfix">
                    <div class="art_title">最近评论</div>
                     <?php if(is_array($comm_info)): $i = 0; $__LIST__ = $comm_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$com): $mod = ($i % 2 );++$i;?><div class="user_list">
                             <?php if(($com["uid"]) == "0"): ?><div class="m_face"><img src="<?php echo getRootUrl();?>Addons/Avatar/default_32_32.jpg"  ></div>
                                 <?php else: ?>
                                <div class="m_face"><img src="<?php echo ($com["face"]["avatar32"]); ?>"  ></div><?php endif; ?><div class="m_left"><a id="link" href="<?php echo U('artc/'.$com['row_id']);?>"><?php echo ($com["content"]); ?></a></div>
...
<div class="copyright">
Kevin--<a href="http://myblog365.xyz/" rel="nofollow">Myblog365</a>博客网&nbsp;&nbsp;&nbsp;&nbsp;Copyright © 2014-2015.
    <br><a href="http://www.miibeian.gov.cn/" rel="nofollow"><?php echo C('WEB_SITE_ICP');?></a> <?php echo hook('pageFooter');?>
</div>
    ...
    <script src="/Public/home/Js/rainbow/rainbow.js"></script>
    <script src="/Public/home/Js/rainbow/language/generic.js"></script>
    <script src="/Public/home/Js/rainbow/language/php.js"></script>
    <script src="/Public/home/Js/rainbow/language/shell.js"></script>
