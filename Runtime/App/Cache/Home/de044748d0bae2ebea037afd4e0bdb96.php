...
<div class="stream-list blog-stream border-top board">
    <?php $map=array($cxuser['uid'],'1','tj desc,id desc','','6','','',);$data = callApi("Focus/getArt",$map);$__LIST__ = $data['data']; if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><section class="stream-list__item">
    <div class="blog-rank">
...
<a callback="focusevent" id="focusevent<?php echo ($vo["id"]); ?>" href="<?php echo U('Index/guanzhu',array('id'=>$vo['id'],'type'=>1));?>" target=AjaxTodo style="color:red;" alt="收藏">取消收藏</a>
...
<?php $map=array($cxuser['uid'],'1','',);$data = callApi("Focus/getArtCount",$map);$count = $data['data'];$__PAGE__ = new \Think\Page($count, 6);echo $__PAGE__->show(); ?>
