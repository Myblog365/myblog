...
<div id="page-content">
    ...
    <form action="<?php echo U('Hooks/update');?>" id="j_custom_form" url="<?php echo U('Hooks/index');?>" class="form-validate col-md-12" method="post" callback="TabAjaxDone" noEnter>
      <input type="hidden" name="id" value="<?php echo ($info["id"]); ?>" />
     
      <div class="form-row">
	 <div class="form-label col-md-2 text-right"><label>钩子名称：</label></div>
          <div class="form-input col-md-6">
            <input size="20" type="text" class="validate[required,custom[NoXhxOneLetter]] required" name="name" id="j_name" value="<?php echo ($info["name"]); ?>"  >
	 ...
	 <div class="form-label col-md-2 text-right"><label>钩子描述：</label></div>
          <div class="form-input col-md-6">
                             <textarea name="description" id="j_description" class="autosize validate[required] required" rows="4" cols="30"><?php echo ($info["description"]); ?></textarea>
	 ...
	 <label>钩子挂载的插件排序：</label>
          ...
          <div class="form-input col-md-4">
					...
					<ul id="sortUl" class="zs-dragsort"  data-selector="li.getSort" data-dragend="updateVal" data-placeholder="#zs_dragsort_placeholder">
						<?php $_result=explode(',',$info['addons']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$addons_vo): $mod = ($i % 2 );++$i;?><li class="getSort bg-azure medium"><b>&times;</b><em><?php echo ($addons_vo); ?></em></li><?php endforeach; endif; else: echo "" ;endif; ?>
					</ul>
	 ...
	 <div class="form-label col-md-2 text-right"><label>钩子类型：</label></div>
          <div class="col-md-6">
                              <select name="type"  data-container="body"  id="j_type" class="selectpicker show-tick validate[required]" data-style="btn medium bg-green" data-width="auto">
                                ...
                                <?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zs): $mod = ($i % 2 );++$i;?><option <?php if($info['type'] == $key): ?>selected<?php endif; ?> value="<?php echo ($key); ?>"><?php echo ($zs); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
