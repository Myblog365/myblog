    ...
    $.noty.themes.agileUI = {
        ...
        style: function() {
            ...
            switch (this.options.type) {
                case 'alert': 
                    this.$bar.addClass('bg-orange');break;
                case 'notification':
                    this.$bar.addClass('bg-blue');break;
                case 'warning':
                    this.$bar.addClass('bg-orange');break;
                case 'error':
                    this.$bar.addClass('bg-red');break;
                case 'information':
                    this.$bar.addClass('bg-gray');break;
                case 'success':
                    this.$bar.addClass('bg-green');break;
                default:
                    this.$bar.addClass('bg-black');break;
            }
        },
        callback: {
            onShow: function() { $.noty.themes.agileUI.helpers.borderFix.apply(this); },
            onClose: function() { $.noty.themes.agileUI.helpers.borderFix.apply(this); }
        }
    };
...
;(function($) {
	$.noty.layouts.topcenter = {
	        ...
	        options: {},
	        container: {
	            object: '<div class="noty-wrapper" id="noty_topcenter" />',
	            selector: 'div#noty_topcenter',
	            style: function() {
	            	$(this).css({'margin':'0','padding':'0','position': 'absolute'});
	            	var width=($(window).width()-294)/2;
	            	var height=$(document).scrollTop()+20;
	        ...
	        parent: {
	            object: '<div />',
	            selector: '.noty_message',
	            css: {}
	        },
	        css: {
	            display: 'none'
	        },
	        addClass: 'content-box bg-white',
...
;(function($) {
    ...
    $.noty.layouts.center = {
        ...
        container: {
            ...
            style: function() {
                $('li',this).addClass('radius-all-4 modal-dialog');
                $(this).css({
                    width: '300px',
                    height: 'auto'
                });
	...
	var methods = {
		...
		*/
		attach: function(userOptions) {
			...
			if (options.binded) {
				...
				// delegate fields
				form.on(options.validationEventTrigger, "["+options.validateAttribute+"*=validate]:not([type=checkbox]):not([type=radio]):not(.datepicker):not([readonly])", methods._onFieldEvent);
				form.on("click", "["+options.validateAttribute+"*=validate][type=checkbox],["+options.validateAttribute+"*=validate][type=radio]", methods._onFieldEvent);
                form.on("ifChecked", "["+options.validateAttribute+"*=validate][type=checkbox],["+options.validateAttribute+"*=validate][type=radio]", methods._onFieldEvent);
		...
		*/
		detach: function() {
            ...
            form.off("ifChecked", "["+options.validateAttribute+"*=validate][type=checkbox],["+options.validateAttribute+"*=validate][type=radio]", methods._onFieldEvent);
		...
		*/
		_validateFields: function(form) {
			...
			if (errorFound) {
				if (options.scroll) {
                    var destination  = 0, fixleft = 0;
                    var offsetParent = first_err.offsetParent();
                    if (!offsetParent.length) {
                        destination = first_err.offset().top;
                        fixleft     = first_err.offset().left;
                    } else {
                        destination = first_err.position().top;
                        fixleft     = first_err.position().left;
                    }
					...
					if (positionType!="bottomRight" && positionType!="bottomLeft") {
						...
						if (prompt_err) {
                            if (!offsetParent.length) {
                                ...
                                destination = prompt_err.position().top;
                            }
						}
					}
					...
					if (options.isOverflown) {
                        var overflowDIV = first_err.closest(options.overflowDIV);
						if(!overflowDIV.length) return false;
                        overflowDIV.animate({ scrollTop: destination }, 1100, function(){
						...
						var scrollContainer = $(options.overflowDIV + ":not(:animated)");
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "condRequired":
						...
						break;
                    case "minTag":
						errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._minTag);
						break;
					case "maxTag":
						errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._maxTag);
						break;
					default:
				}
		...
		*/
		_handleStatusCssClasses: function(field, options) {
		 ...
		 _validityProp: {
             ...
             "minTag": "range-underflow",
			 "maxTag": "range-overflow"
		 },
		/**
		* Required validation
		*
		...
		_required: function(field, rules, i, options, condRequired) {
			switch (field.prop("type")) {
				...
				default:
                    ...
                    /*tags 标签专用验证*/
                    if (field.hasClass('tags-control')) {
                        var field_val      = $.trim( field.find(':hidden').val()               );
                        var dv_placeholder = $.trim( field.attr("data-validation-placeholder") );
                        var placeholder    = $.trim( field.attr("placeholder")                 );
                        if (
                               ( !field_val                                    )
				...
				case "checkbox":
					...
					break;
				case "button":
					//button only for bootstrap-select by K'naan
					var field_val      = $.trim( field.parent().prev().val()               );
					var dv_placeholder = $.trim( field.attr("data-validation-placeholder") );
					var placeholder    = $.trim( field.attr("placeholder")                 );
					if (
						   ( !field_val                                    )
		...
		*/
        ...
        /**自定义tag 标签验证**/
        _maxTag: function(field, rules, i, options) {
			var max = rules[i + 1];
            var len = field.find(':hidden').length;
			if (len > max) {
				var rule = options.allrules.maxTag;
				return rule.alertText + max + rule.alertText2;
			}
		},
        _minTag: function(field, rules, i, options) {
			var min = rules[i + 1];
            var len = field.find(':hidden').length;
			if (len < min) {
				var rule = options.allrules.minTag;
				return rule.alertText + min + rule.alertText2;
			}
		},
		...
		_buildPrompt: function(field, promptText, type, ajaxed, options) {
			...
			if (options.showArrow) {
				...
				switch (positionType) {
					...
					case "Right":
						prompt.find(".formErrorContent").before(arrow);
						prompt.find(".formErrorContent").before('<div class="tooltip-arrow"></div>');
						//arrow.addClass("formError").html('<div class="tooltip-arrow"></div>');
						break;
					case "topLeft":
					case "topRight":
...
if(prompt.find('.formErrorContent').height()>25){
			prompt.find('.tooltip-arrow').css({'top':'20%'});
}
		...
		*/
		_calculatePosition: function (field, promptElmt, options) {
            var parent = $(field).offsetParent(); //$(field).closest(options.overflowDIV);
			var promptTopPosition, promptleftPosition, marginTopSize;
			var fieldWidth 	 = field.width();
			var fieldLeft 	 = field.position().left;
			var fieldTop 	 = field.position().top + parent.scrollTop();
			...
			switch (positionType) {
				...
				case "centerRight":
					promptTopPosition = fieldTop + 3;
					marginTopSize = 0;
					//promptleftPosition= fieldLeft + field.outerWidth(true)+5;
                    promptleftPosition= fieldWidth - promptElmt.width() ;//- (promptElmt.width() + 2);
					break;
				case "centerLeft":
					...
					break;
				case "Right":
					promptTopPosition +=  fieldTop+promptHeight;
					
					promptleftPosition += fieldWidth+fieldLeft+60;
				...
				case "bottomRight":
					if (fieldWidth > 105) {
						promptleftPosition = fieldLeft + fieldWidth - 105;
					} else {
						promptleftPosition = fieldLeft + Math.round(fieldWidth / 2);
					}
					promptTopPosition =  fieldTop + field.height() + (field.attr('errTop') ? parseInt(field.attr('errTop')) : 4);
					marginTopSize = 0;
					break;
				case "inline":
	    ...
	    $.validationEngineLanguage = {
	        newLang: function(){
	            $.validationEngineLanguage.allRules = {
	                ...
	                "NoXhxOneLetter": {
	                    "regex": /^[^_{1}][a-zA-Z_\ \']+$/,
	                    "alertText": "* 接受英文字母大小写、下划线，但下划线不能在最前"
	                },
	                "onlyLetterNumber": {
	                    "regex": /^[0-9a-zA-Z]+$/,
	                    "alertText": "* 不接受特殊字符"
	                ...
	                //K'naan@2014-04-25 自定义日期时间格式
	                ,"dateTime": {
	                	//"regex": /^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])\s+(2[0123]|1[0123456789]|0?[1-9]){1}:(0?[1-5]|[0-6][0-9]){1}:(0?[0-6]|[0-6][0-9]){1}$|^(?:(?:(?:0?[13578]|1[02])(\/|-)31)|(?:(?:0?[1,3-9]|1[0-2])(\/|-)(?:29|30)))(\/|-)(?:[1-9]\d\d\d|\d[1-9]\d\d|\d\d[1-9]\d|\d\d\d[1-9])$|^((1[012]|0?[1-9]){1}\/(0?[1-9]|[12][0-9]|3[01]){1}\/\d{2,4}\s+((2[0123])|(1[0123456789])|0?[1-9]){1}:(0?[1-5]|[0-6][0-9]){1}:(0?[0-6]|[0-6][0-9]){1}$/,
	                	"regex": /^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])\s+(2[0123]|(1|0?)[0-9]){1}:(0?[1-5]|[0-6][0-9]){1}:(0?[0-6]|[0-6][0-9]){1}$|^(?:(?:(?:0?[13578]|1[02])(\/|-)31)|(?:(?:0?[1,3-9]|1[0-2])(\/|-)(?:29|30)))(\/|-)(?:[1-9]\d\d\d|\d[1-9]\d\d|\d\d[1-9]\d|\d\d\d[1-9])$|^((1[012]|0?[1-9]){1}\/(0?[1-9]|[12][0-9]|3[01]){1}\/\d{2,4}\s+(2[0123]|(1|0?)[0-9]){1}:(0?[1-5]|[0-6][0-9]){1}:(0?[0-6]|[0-6][0-9]){1})$/,
	                    "alertText": "* 无效的日期时间格式，yyyy-mm-dd hh:mm:ss"
	                },
	                "mobile":{
     ...
     Selectpicker.prototype = {
         ...
         createA: function(text, classes, inline) {
             return '<a tabindex="0" class="'+classes+'" style="'+inline+'">' +
                  text +
                  '<i class="' + this.options.iconBase + ' icon-check check-mark"></i>' +
                  '</a>';
         },
			...
			var newList = {
				...
				setPos: function(x, y) { 
					...
					this.draggedItem.parents().each(function() {
						if ($(this).css("position") != "static" && (userBrowser()!='Firefox' || $(this).css("display") != "table")) {
				...
				wheel: function(e) {
					if ((userBrowser()=='Safari' || userBrowser()=='Firefox') && list && opts.scrollContainer != window) {
						var cont = $(opts.scrollContainer);
						var offset = cont.offset();
...
!function ($) {
	...
	var Datetimepicker = function (element, options) {
		...
		this.icons = {
			leftArrow: this.fontAwesome ? 'fa-arrow-left' : (this.bootcssVer === 3 ? 'glyphicon-arrow-left' : 'glyph-icon icon-arrow-left'),
			rightArrow: this.fontAwesome ? 'fa-arrow-right' : (this.bootcssVer === 3 ? 'glyphicon-arrow-right' : 'glyph-icon icon-arrow-right')
		};
		...
		if ('minView' in options) {
			...
			//alert(options.minView);
			//alert(this.element.data('minView'));
	...
	Datetimepicker.prototype = {
		...
		show: function (e) {
			...
			this.container.getNiceScroll().resize();
			this.place();
